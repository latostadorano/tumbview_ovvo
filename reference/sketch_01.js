...
function mousePressed() {
  if (mouseX < windowWidth / 2) {
    // Clicked on the left half, go back one image
    goBack();
  } else {
    // Clicked on the right half, go forward one image
    ask();
  }
}
...
function goBack() {
  ...
  // Adjust the postsIndex to go back one image
  postsIndex = (postsIndex - 1 + postsPhotoArray.length) % postsPhotoArray.length;
  stopTimer = true; // Stop the timer to prevent multiple calls
  ask(); // Fetch and display the previous image
}
...
function randomImg() {
  if (noMorePhotosLeft == false) {
    ...
    if (totalPhotosRemaining <= 0) {
      ...
      offset++; // Offset is the number that handle the block of posts request (50 each)
      newData();
